...
object YouTube {
    ...
    /**
     * @param incognito when true the request is sent anonymously, so the query is not recorded in
     * the user's YouTube search history. Used for background Spotify→YouTube matching.
     */
    suspend fun searchSummary(query: String, incognito: Boolean = false): Result<SearchSummaryPage> =
        runCatching {
            val response =
                innerTube
                    .search(WEB_REMIX, query, setLogin = if (incognito) false else null)
                    .body<SearchResponse>()
